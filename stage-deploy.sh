...
ssh -o StrictHostKeyChecking=no ubuntu@${STAGE_SERVER} 'docker run -d -e "POSTGRES_USER=postgres" -e "POSTGRES_PASSWORD=postgres" -p 5433:5432 --name users-db --network users-service-network registry.gitlab.com/gusisoft/onelike/client/users-service/users-db:devel'
ssh -o StrictHostKeyChecking=no ubuntu@${STAGE_SERVER} 'docker run -d -e "API_URL=definitions/users-service.yml" -p 8081:8080 --name users-swagger --network users-service-network registry.gitlab.com/gusisoft/onelike/client/users-service/users-swagger:devel'
