...
ssh -o StrictHostKeyChecking=no ubuntu@${STAGE_SERVER} 'docker run -d -e "API_URL=definitions/users-service.yml" -p 8081:8080 --name users-swagger --network users-service-network registry.gitlab.com/gusisoft/onelike/client/users-service/swagger:devel'
